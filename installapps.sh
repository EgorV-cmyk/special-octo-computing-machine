...
[ -z "$REPO" ] || [ -z "$PACKAGE" ] && {
  log_error "Usage: $0 <repo> <package>"
  exit 1
}
...
download_apk() {
  ...
  curl -s -O "$url" || { log_error "Error loading $PACKAGE"; exit 1; }
  log_info "Downloaded $(du -h "$APK_FILE" | cut -f1) as $APK_FILE"
}

case "$REPO" in
  main_community)
    download_apk "$APK_URL_MAIN_COMMUNITY"
    ;;
  edge_community)
    download_apk "$APK_URL_EDGE_COMMUNITY"
    ;;
  testing)
    download_apk "$APK_URL_TESTING"
    ;;
  main)
    download_apk "$APK_URL_MAIN"
    ;;
  *)
